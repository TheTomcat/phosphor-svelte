<script lang="ts">
	...
	const visible = $derived(
		textOpts.preserveSpacing ? text.substr(0, char).replaceAll(' ', '&nbsp;') : text.substr(0, char)
	);
	const cursor = $derived(text.substr(char, 1) || ' ');
	const hidden = $derived(text.substr(char + 1));
...
<div class={`__teletype__ ${className ? className : ''}`}>
	...
	<span class="cursor" bind:this={_cursorRef}>{cursor}</span>
